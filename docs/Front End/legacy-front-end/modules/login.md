...
* `user` (object|false) – when present, logs in *as* the given user id (admin/impersonation flows).
...
* `checkValidation()`\
  ...
  On success → clears `validate` cache and calls `finishLogin(token, uid)`; on “already\_claimed” → clears cache; otherwise re-polls every 1s.
...
* **Password path gating:** The password box is *suppressed* unless `app.debugallowpassword` is true—intended for store-tester or internal flows.
