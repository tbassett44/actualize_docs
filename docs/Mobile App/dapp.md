...
Create a shell shortcut on your system with this logic.  Can be an alias in you .bash\_profile or .zshrc or as a file in /usr/local/bin
...
* **app\_name**: usually `actualize` (maps to `/config/[app].json`).
...
* `createsns` → Creates AWS SNS platforms for APNS/APNS\_SANDBOX (and later Android).
...
* `getconf=save`
* **`updatetemplate`** *(listed but no handler—see “Holes to poke” below)*
* `incbuild`
* `chdir`
* `copyresources`
* `ensurenpm`
...
* `createconfig`
* `fixxml type=ios`
* **`devcert`** *(listed but no handler—see “Holes to poke” below)*
* `ensurehooks`
* `buildios`
...
* `getconf=save`
* **`updatetemplate`** *(listed but no handler—see “Holes to poke”)*
* `incbuild`
* `chdir`
* `copyresources`
* `ensurenpm`
...
* `createsns` → AWS SDK: creates APNS / APNS\_SANDBOX platform apps (and later with `androidsns` for GCM/FCM) using the certs/keys you just created.
...
* **createsns**: Create APNS/APNS\_SANDBOX SNS platform apps with current certs/keys.
...
2. **`ensureiosapp`conditional**\
   In `runCommands()` it only runs when `isset($v['ensureiosapp']) && self::$force != 2`. If you pass `force==2` to `create`, the earlier `createstart` nukes keystores—but `ensureiosapp` then *won’t* rebuild certs. If that’s intentional, great; otherwise you’ll strand a fresh app without certs.
