...
`admin` — Runs PHP CLI admin action via classes/admin_cli.php (usage: admin [args]).  
`backupdb2` — Dumps a MongoDB database using credentials in ~/priv/config_auto.json (DB name placeholder XXXXX).  
...
`createpasscode` — Creates an AES‑256 key at ~/priv/aes_key (and backups to /home/aes_key) if absent.  
`db2` — Opens a Mongo shell using env credentials from ~/priv/config_auto.json (auth DB admin).  
